...
# use slog::{Drain, o};
# use std::sync::Arc;
# use raft::{logger::Slogger, Config, storage::MemStorage, raw_node::RawNode};
...
```rust
# use raft::{logger::Slogger, Config, storage::MemStorage, raw_node::RawNode, eraftpb::Message};
# use std::{
#     sync::Arc,
...
other nodes:
    ...
    # use slog::{Drain, o};
    # use std::sync::Arc;
    # use raft::{logger::Slogger, Config, storage::MemStorage, raw_node::RawNode, StateRole};
    #
    # let config = Config { id: 1, ..Default::default() };
    # config.validate().unwrap();
    # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
...
need to update the applied index and resume `apply` later:
    ...
    # use slog::{Drain, o};
    # use std::sync::Arc;
    # use raft::{logger::Slogger, Config, storage::MemStorage, raw_node::RawNode, eraftpb::EntryType};
    #
    # let config = Config { id: 1, ..Default::default() };
    # config.validate().unwrap();
    # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
...
```no_run
# use std::sync::Arc;
# use raft::{logger::Slogger, Config, storage::MemStorage, raw_node::RawNode, eraftpb::*};
